...
// # This drill cannot saturate on a two-core runner, and concurrency
// # does not fix it. Tried 2026-09-11; do not try it again blind.
//
// The offer rate has to beat the ceiling per *block*. On a developer's
// machine one attempt costs ~26ms and the loop produces ~40 offers a
// block, comfortably past the two dozen outputs the wallet holds, so the
// ceiling is reached and §6.4b's one-per-block is refuted.
//
// On the runner the nightly uses, the same attempt costs ~1.5s: node,
// miner and hub compete for two cores and the proof of work is on top.
// The 2026-09-10 run offered 2.91 payouts a block, granted every one,
// and was refused for balance exactly zero times -- it never reached any
// ceiling and correctly reported `inconclusive`.
//
// The obvious fix is to stop waiting for each round trip, so this loop
// was rewritten to keep eight claims in flight. That made it strictly
// worse: on the runner the faucet requests time out
// (`POST /faucet: operation timed out`) and the drill fails before
// writing a report at all. A truthful "cannot tell" became a hard error,
// which is a worse thing to publish, so it was reverted.
//
// What would actually help is making an attempt cheaper rather than
// running more of them at once -- the proof of work dominates, and the
// stack runs it at 64 expected hashes. Until then this drill is a
// developer-machine instrument, and its `inconclusive` on CI is the
// honest reading rather than a defect to paper over.
