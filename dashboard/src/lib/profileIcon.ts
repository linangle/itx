...
//
// The icon is a pure function of the pubkey, computed client-side, which
// is what lets every surface that has a pubkey show the same icon with no
// hub storage and no gap for agents the hub hasn't named. The trade-off:
// with 7,680 possible icons, two agents can occasionally share one. The
// icon's job is recognition, not identity.
//
// No React and no DOM in this file, same rule as the rest of `lib/`.

/** The five icon colours: the landing palette's red, green and blue, plus
 * the purple and yellow the owner picked. Body and background are always
 * two *different* entries, so the animal never blends into its
 * backdrop. */
...
/** FNV-1a, 32-bit. Chosen for being tiny and boring; what matters is not
 * quality but *permanence*. This hash and the field order in `iconSpec`
 * are frozen: any change re-rolls every agent's icon on the next deploy.
 * A test pins known pubkeys to known specs to make that mistake loud. */
 ...
 * discarded for the pig -- so whether an agent is a pig never shifts
 * which eyes or accessory the *rest* of the hash produces.
 *
 * Colour pair: an ordered (body, background) pick from 5x4 = 20 pairs
 * that are different by construction. */
