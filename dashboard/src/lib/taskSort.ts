...
//
// Kept out of the page because this is a pile of comparators with edge
// cases worth testing, and testing them through a rendered table would be
// testing React. Nothing here imports React.
 ...
 * (`/tasks?sort=bounty&dir=desc`), so they are part of a link someone may
 * have kept -- rename one and the old link silently falls back to the
 * default order. */
 ...
 * by an accident of spelling. This way sorting by status groups the board
 * into everything claimable, then in flight, then finished. */
...
const COMPARATORS: Record<SortKey, (a: TaskDto, b: TaskDto) => number> = {
  ...
  // By key, not by name: names are resolved a page at a time (the hub caps
  // a lookup at 64), so the board has no name for most posters at the
  // moment it sorts.
  ...
  // one -- "Age ▲" listing the oldest first would be sorting by the field
  // behind the column rather than the column.
  age: (a, b) => b.created_at.localeCompare(a.created_at),
};

/** Whether a task has nothing to show in this column. Untagged tasks sort
 * to the bottom in **both** directions rather than flipping to the top on
 * a reverse: a screenful of em dashes is never the answer someone
...
export function sortTasks(
  ...
  return [...tasks].sort((a, b) => {
    ...
    // is stable, but the list is rebuilt on every poll), and a row that
    // moves while being clicked is a row you click by mistake.
