 ...
 * ground rules allow. Since we already fetch the whole board for the
 * overview's aggregates, filtering in the browser costs nothing extra
 * here. If the board ever outgrows a single fetch, a server-side kind
 * filter is the fix, not a bigger page cap. */
export default function TasksPage() {
  ...
  const visible: TaskDto[] = (tasks.data?.items ?? [])
  ...
  return (
    <Shell>
      ...
      <div className="itx-filters">
        ...
        {tasks.data && (
          <span className="flat" style={{ fontSize: 12 }}>
      ...
      <section className="itx-panel">
        ...
        {tasks.data && visible.length === 0 && <Empty>No tasks match these filters.</Empty>}
