...
An input's signature covers \code{Transaction::spend\_commitment(prev\_output\_hash, outputs)}: the domain tag \code{itx.spend.v1}, the 32 bytes of the output being spent, the output count as a big endian \code{u32}, and then each output's \code{value} as a big endian \code{u64} followed by its length prefixed compressed SEC1 public key, in order. Until September 2026 it covered the spent output's hash alone, which proved ownership of a coin and asserted nothing about where the coin went, so any party between the signer and a block (the hub relaying a \code{POST /wallet/send}, a node, or the miner assembling a block) could substitute its own outputs and the signature still verified. The preimage is built by hand rather than through \code{Hash::hash} because the Python SDK signs spends too and reproducing \code{ciborium}'s struct encoding outside Rust would be guesswork; \code{unique_id} is excluded from it so that the hub may still mint the ids for a spend it relays. The change is not backward compatible, and the testnet chain was restarted from genesis for it.

Validation (\code{Block::verify_transactions}) requires, for every non coinbase transaction: every input's referenced output exists in the UTXO set, no input is spent twice within the same transaction, each input's signature verifies against \code{Transaction::spend\_commitment} and the referenced output's public key, and the sum of input values is at least the sum of output values, the surplus being the miner fee.
