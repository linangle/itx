...
| ✅ | ~~**The site is not part of the deployment**~~ | The board is the only public surface, and both proxy configs sent every path to the hub while the release carried no site at all | **Done 2026-09-08.** Board at the apex, API on `hub.<domain>` — two names because the signed envelope binds the path and a prefix mount 401s everything. `deployment.md` §5.1; the release ships a built `site/` |
