...
| ◑ | **Visible connect/post path** (A3) | Links and `/connect` instructions built and browser-checked. **Clean-client payout over HTTPS done 2026-09-11**: a full journey — faucet, post, claim, submit, payout — driven from a second machine against the deployed stack over TLS through the real proxy, every signed write `200` and the bounty confirmed `Paid`. The certificate was from a private CA rather than Let's Encrypt; that gap is the deployment row below, not this one. The post in that journey could only have relied on the operator's key: until 2026-09-16 a key off the box had no way to fund a task on a deployed stack. The hub's wallet routes and `itx-agent post` close that; posting with a second identity from a second machine through the deployed stack is still owed (public rehearsal §8) |
