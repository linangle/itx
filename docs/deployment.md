...
| a payout that is stuck rather than merely in flight | **yes** | `hub_board_oldest_payout_attempt_seconds`, sampled beside the count. The depth alone cannot separate a busy hub from a stalled one, and the stall is otherwise silent — see the alert row in §8.4 |
...
| replay guard / commit latency | **yes** | `hub_replay_signatures_{claimed,rejected,evicted}_total`, `hub_replay_durable_write_failures_total`, and `hub_replay_durable_write_ms_total` — the last is the fsync that bounds the whole write path (plan §6.3). `hub_replay_guard_degraded` says the guard came up unable to read its log, which was silent and permanent before 2026-09-07 |
...
| faucet burn *in units* | **yes** | `hub_faucet_granted_units`, also exposed as `faucet.granted_units_all_time` in the admin overview. Corrected by the 2026-09-10 audit: this signal is implemented; challenge issuance-to-redemption conversion remains missing |
...
| Faucet budget exhausted | `hub_faucet_budget_remaining == 0` | the faucet is refusing everyone. Either an attack is being absorbed exactly as intended, or `--faucet-daily-grants` is set below what an honest cohort needs — and the two look identical from here. `hub_faucet_grants` rising alongside says it is absorbing; flat says the budget is simply too low. The per-network price never refuses, so a shared network never shows up here on its own; the per-block share (`--faucet-network-share-percent`, a /16 or /48 may hold at most that much of the budget, 25% by default) does refuse, with its own message naming the block, and it is what keeps one tenant's /48 from spending this whole budget at the base price and turning every later arrival away |
| Payment needs a human | `hub_payments_needs_review > 0` | a faucet grant, withdrawal or escrow disbursement the hub has proven it cannot complete: either every resend was refused, or the chain says no block holds it and its inputs are gone. Nothing further happens automatically. §10.4's procedure applies to all three, not only withdrawals |
| Payment stuck pending | `hub_payments_oldest_pending_seconds > 900` | a payment neither confirmed nor abandoned for fifteen minutes. Expect brief excursions — a payment is pending by design until the sweep sees it on chain. A value that climbs without ever falling means the evidence scan is not reaching the node (check `hub_node_connect_failures_total`), because a scan that *can* read the chain always terminates in either confirmation or review |
| Task payout stuck | `hub_board_oldest_payout_attempt_seconds > 900` | a bounty the hub sent and has been waiting on for fifteen minutes. The count beside it (`hub_board_outstanding_payouts`) cannot say this: a steady three is a healthy hub paying three agents, and a steady three where the same three have waited forty minutes is a stalled miner — both read as `3`. Nothing else says it either. `HeldInMempool` is deliberately unlogged (every payment passes through it, so a line there is one per payment per minute), and each sweep's resend looks like ordinary retry noise in the journal. Left alone, the node evicts at ten minutes, the hub reads `NeverLanded`, resends the same bytes, and about forty minutes later the four-submission budget is spent and the task is `PayoutFailed` (§9.10). An age that only climbs is the whole signal |
| Withdrawal unresolved | `increase(hub_unresolved_withdrawals_total[10m]) > 0`, and `hub_unresolved_withdrawals_at_boot > 0` | a withdrawal reached the node and was never acknowledged, so the ledger stays debited with the coin's fate unknown. §10.4 is the procedure. The boot gauge is the same thing found on disk at startup |
| Ledger and store disagree | `increase(hub_withdrawal_reverts_not_persisted_total[10m]) > 0` | a withdrawal that was never built, whose credit-back could not be written. The in-memory balance is the true one and the stored one is short by the withdrawn amount, so **a restart takes that money from the user**. Different from the row above and needing the opposite reaction: nothing moved on chain, the store is what is broken (§9.5), and the fix is to get the disk healthy *before* restarting, not after |
| Replay guard running blind | `hub_replay_guard_degraded > 0` | this process could not read its replay log at boot. It still records durably and still refused writes for its first four minutes, so nothing is open right now — but it lost the previous process's history, and until 2026-09-07 this state was both permanent and invisible. Restart it once the disk is healthy; the gauge cannot clear on its own |
